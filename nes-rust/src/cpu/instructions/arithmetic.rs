...
define_instruction!(SBC, |cpu: &mut CPU, _: &mut MemoryBus, value: u8| {
...
define_instruction!(CPX, |cpu: &mut CPU, _: &mut MemoryBus, value: u8| {
    cpu.set_flag(Status::CARRY, cpu.get_x() >= value);
    let result = cpu.get_x().wrapping_sub(value);
    cpu.update_zero_and_negative_flags(result);
});

define_instruction!(CPY, |cpu: &mut CPU, _: &mut MemoryBus, value: u8| {
