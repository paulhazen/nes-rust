...
define_instruction!(PHA, |cpu: &mut CPU, memory: &mut CPUBus, _: u8| {
    cpu.push_stack(memory, cpu.get_a());
});

// Push Processor Status onto Stack (PHP)
define_instruction!(PHP, |cpu: &mut CPU, memory: &mut CPUBus, _: u8| {
...
define_instruction!(PLA, |cpu: &mut CPU, memory: &mut CPUBus, _: u8| {
...
define_instruction!(PLP, |cpu: &mut CPU, memory: &mut CPUBus, _: u8| {
